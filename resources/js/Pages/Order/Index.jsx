...
export default function OrderIndex({ initialOrders = {}, summary = {} }) {
    ...
    const getStatusDisplay = (orderStatus, paymentStatus, returnedAmount = 0) => {
        ...
        if (orderStatus === 'preparing' || orderStatus === 'processing') {
            return {
                primary: { label: 'Diproses', color: 'bg-yellow-400 text-white', icon: FiAlertCircle },
                secondary: isPaid
        ...
        <AuthenticatedLayout pageTitle={locale === 'en' ? 'Orders List' : 'Daftar Pesanan'}>
            ...
            <div className="w-full space-y-4 p-3.5 sm:p-5">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3.5">
                    <div className="bg-white dark:bg-slate-900 p-3.5 sm:p-4 rounded-xl border border-slate-200/80 dark:border-slate-800 shadow-2xs transition-colors">
                        <p className="text-[10px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-wider">TOTAL PESANAN HARI INI</p>
                        <p className="text-2xl font-black text-slate-900 dark:text-white mt-1">{todayOrderCount}</p>
                    </div>
                    <div className="bg-white dark:bg-slate-900 p-3.5 sm:p-4 rounded-xl border border-slate-200/80 dark:border-slate-800 shadow-2xs transition-colors">
                        <p className="text-[10px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-wider">NILAI PESANAN HARI INI</p>
                        <p className="text-2xl font-black text-slate-900 dark:text-white mt-1">{formatRp(todayOrderValue)}</p>
                    </div>
                    <div className="bg-white dark:bg-slate-900 p-3.5 sm:p-4 rounded-xl border border-slate-200/80 dark:border-slate-800 shadow-2xs transition-colors">
                        <p className="text-[10px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-wider">MENUNGGU</p>
                        <p className="text-2xl font-black text-slate-900 dark:text-white mt-1">{pendingCount}</p>
                    </div>
                    <div className="bg-white dark:bg-slate-900 p-3.5 sm:p-4 rounded-xl border border-slate-200/80 dark:border-slate-800 shadow-2xs transition-colors">
                        <p className="text-[10px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-wider">DIPROSES</p>
                        <p className="text-2xl font-black text-slate-900 dark:text-white mt-1">{processingCount}</p>
                    </div>
                </div>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xs border border-slate-200/80 dark:border-slate-800 overflow-hidden transition-colors">
                    ...
                    <div className="p-3.5 sm:p-4 border-b border-slate-200 dark:border-slate-800 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-3 bg-slate-50/50 dark:bg-slate-800/50">
                        <h3 className="text-base font-black text-slate-900 dark:text-white shrink-0">Daftar Transaksi</h3>
                        ...
                        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 w-full lg:w-auto">
                            ...
                            <div className="relative w-full sm:w-72">
                                ...
                                <input
                                    ...
                                    placeholder="Cari pesanan... (Ctrl+F)"
                                    className="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg pl-9 pr-3 py-1.5 text-xs text-slate-800 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all"
                                />
                            </div>
                        </div>
                    </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-slate-100/90 dark:bg-slate-800/90 border-b border-slate-200 dark:border-slate-800 text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-wider sticky top-0 z-10">
                                    <th className="px-3.5 py-2.5">ID PESANAN</th>
                            ...
                            <tbody className="divide-y divide-slate-200 dark:divide-slate-800 border-b border-slate-200 dark:border-slate-800">
                                {pageOrders.map(order => {
                                    ...
                                    const sc = statusConfig[order.status] || { label: order.status || 'Menunggu', icon: FiClock, color: 'bg-slate-600 text-white' };
                                    ...
                                    return (
                                        <tr key={order.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
                                            {/* ID PESANAN */}
                                            <td className="px-3.5 py-2.5 font-mono text-xs font-bold text-slate-700 dark:text-slate-300">{order.id}</td>
                                            ...
                                            <td className="px-3.5 py-2.5">
                                                <span className="px-2 py-0.5 rounded-md text-[10px] font-black tracking-wider uppercase bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
                                                    {order.type} {order.table !== '-' ? `· ${order.table}` : ''}
                                                </span>
                                            </td>

                                            {/* JUMLAH ITEM */}
                                            <td className="px-3.5 py-2.5 text-xs font-bold text-slate-700 dark:text-slate-300">{order.items}x</td>
                                            ...
                                            {/* WAKTU */}
                                            <td className="px-3.5 py-2.5 text-[11px] font-medium text-slate-500 dark:text-slate-400">{order.time}</td>
                                ...
                                {pageOrders.length === 0 && (
                                    <tr>
                                        <td colSpan={8} className="px-5 py-12 text-center text-sm text-slate-400 dark:text-slate-500 font-semibold">
                    ...
                    <div className="p-3.5 sm:p-4 bg-slate-50/80 dark:bg-slate-800/60 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs font-semibold text-slate-600 dark:text-slate-400">
                        <span>
                            Menampilkan {totalItems > 0 ? `${(currentPage - 1) * perPage + 1} - ${Math.min(currentPage * perPage, totalItems)}` : '0'} dari {totalItems} pesanan
                        </span>
                        ...
                        <div className="flex items-center space-x-2">
                            <button
                                ...
                                className="px-3.5 py-1.5 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 disabled:opacity-40 disabled:hover:bg-white border border-slate-300 dark:border-slate-700 rounded-lg font-bold text-slate-700 dark:text-slate-200 transition flex items-center gap-1 shadow-2xs text-xs"
                            >
                                <FiChevronLeft size={14} strokeWidth={2.5} />
                                <span>Sebelumnya</span>
                            </button>
                            
                            <span className="px-3 py-1.5 bg-slate-100 dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg font-bold text-slate-800 dark:text-slate-200 text-xs">
